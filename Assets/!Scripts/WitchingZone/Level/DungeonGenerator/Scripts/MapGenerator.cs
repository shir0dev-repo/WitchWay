...
namespace DungeonMaster2D
{
    ...
    {
        public static Dungeon2D Generate2D(DungeonGeneratorData data, NodeType[] specialTypes)
        {
            ...
            while (currentIteration < 30)
            {
                ...
                int attempts = 0;
                // check dungeon.MaxNodes - 1 because of starting room
                while (attempts < 300 && roomCount < dungeon.MaxNodes - 1 && deadendQueue.TryDequeue(out Node currentNode))
                {
        ...
        private static bool CanEarlyOutGeneration(int roomCount, Dungeon2D dungeon, int deadEndRequirement)
        {
            return roomCount >= dungeon.MinNodes && dungeon.GetDeadends().Length >= deadEndRequirement;
        }
